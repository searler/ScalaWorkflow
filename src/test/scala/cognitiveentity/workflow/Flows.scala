 ...
 *  being accessed via a bound variable. This is not idiomatic for
 * inject.
 *  Demonstrates how per flow state can be used.
 */
...
/**
 * Same as TwoLineBalanceSumVar but is idiomatic since it uses
 * the value generated by inject.
...
object TwoLineBalanceVarying{
    def apply(pn:Num)(implicit acctLook:Lookup[Num,Acct],  balLook:Lookup[Acct,Bal]) = {
       val  process = {var total = Bal(0);b:Bal => total += b;total}
       Flow.inject(process)(
...
object TwoLineBalanceEfficient{
    def apply(pn:Num)(implicit acctLook:Lookup[Num,Acct],  balLook:Lookup[Acct,Bal]) = {
       val  process = {var total = Bal(0);b:Bal => total += b;total}
        acctLook(pn){a:Acct=>Flow.inject(process)(
          c => balLook(a)(c) ,
          c => balLook(a)(c) )(End)
       }
    }
...
/**
 * Extends TwoLineBalanceEfficient, demonstrating how logic can be
 * included in the next function of inject.
 */
...
object PrepaidAndBalance{
    def apply(pn:Num)(implicit acctLook:Lookup[Num,Acct],  balLook:Lookup[Acct,Bal], ppLook:Lookup[Acct,PP]) = {
      val  process = {var list = new scala.collection.mutable.ListBuffer[BalanceLike];b:BalanceLike => list+=b;list toList}
      Flow.inject(process)(
        c => acctLook(pn){balLook(_){c}} ,
